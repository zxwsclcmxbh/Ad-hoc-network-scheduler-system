...
def trace(type,message_id):
    cerebellum_addr="cerebellum-svc-"+settings.node_name+":3000"
    Thread(target=requests.post,args=(cerebellum_addr+"/api/v1/report/trace",),kwargs={"json":{"type":type,"message_id":message_id,"task":settings.task_id,"pod":settings.pod_name,"time":time.time_ns()}}).start()